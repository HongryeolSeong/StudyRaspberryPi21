...
while True:
    ...
    if key == ord('q'): break
        ...
        is_record = True
        video = cv2.VideoWriter('./0719_20_21/capture/record_{0}.avi'.format(fileDateTime), fourcc, 20, (w, h))
        print('녹화 시작')
    ...
    if is_record:
        video.write(frame)
        cv2.circle(img=frame, center=(620, 15), radius=5, color=(0, 0, 255), thickness=4)
